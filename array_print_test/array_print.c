...
void array_print_bang(t_array_print *x)
{
  ...
  t_word* buf;
  int maxindex;
  dsparray_get_array(x->x_v.v_vec, &maxindex, &buf, 0);
...
static void array_print_set(t_array_print *x, t_symbol *s, int argc, t_atom *argv)
{ 
  arrayvec_set(&x->x_v, argc, argv);
}


static void array_print_free(t_array_print *x)
{
  arrayvec_free(&x->x_v);
}
...
void array_print_setup(void) {
  /* create a new class */
  array_print_class = class_new(gensym("array_print"),        /* the object's name is "array_print" */
